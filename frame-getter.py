...
for video in files:
  name = uuid.uuid1().int
  print(system(f'ffmpeg -i "{directory}\\{video}" -vf "select=\'if(not(floor(mod(t,5)))*lt(ld(1),1),st(1,1)+st(2,n)+st(3,t));if(eq(ld(1),1)*lt(n,ld(2)+8),1,if(trunc(t-ld(3)),st(1,0)))\'" -vsync 0 {dir_output}{name}_out%d.png'))
